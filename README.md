...
> **一套纯 numpy 的声明式因子语言 + 一个遗传编程搜索器 + 一个 妙算 策略双向适配层。**
...
| 模式 | 输入 | 输出 | 引擎 |
|---|---|---|---|
| **模式 A：因子挖掘** | 行情数据（Panel） | 一份可信的因子公式 → 可导出的 妙算 策略 | RPN-GA 结构搜索 |
| **模式 B：参数寻优** | 已有 妙算 策略 | 外提参数后的最优参数集 → 回写策略新版本 | Optuna TPE + 事件驱动回测 |
...
CLI  ──►  Pipeline  ──►  {search(GA) | tune(Optuna)}  ──►  core(纯 numpy)
                  ...
                  data/ 下的数据获取模块统一封装「妙算本地库 + 网络下载」两类来源（§3.2）
```
...
妙算（MiaoSuan）与妙算（Shenji）是**配套套装**，二者分工明确、通过统一接口解耦：

- **妙算**：策略运行与执行侧。**正式上线部署运行在妙算侧**——实盘 / 半实盘交易、信号分发、实时行情接入均在妙算后台完成。
- **妙算**：策略研究与生产侧。负责因子挖掘、参数寻优、回测、纸面交易仿真，产出可导出的妙算策略。

妙算的产物（妙算策略 `.py`）经 `export` 导出后交由妙算加载运行；妙算运行产生的绩效、信号、实时行情又回流为妙算回测与纸面交易的输入。两者通过统一的 `StrategySpec` IR 与数据获取模块解耦，**互不内嵌**。
...
| 来源 | 说明 | 封装方式 |
|------|------|----------|
| **① 妙算本地数据库** | 妙算侧已落库的行情（首选、最低延迟） | 经妙算提供的只读查询接口 / 本地 DB 驱动拉取 |
| **② 网络下载** | 从数据服务商 / 公开源按需下载 | 经封装好的 HTTP/SDK 客户端拉取 |

**职责边界**：

- 数据获取模块**只负责"取数"**：统一抽象两类来源、处理认证、缓存落地、增量 / 全量决策。**不**做特征计算、不碰策略逻辑、不写回妙算。
...
- 数据获取模块**随妙算部署**，但正式生产环境的"数据源主干"指向**妙算本地数据库**；网络下载作为补充 / 灾备通道。
- 妙算侧需为妙算开放**只读**数据查询权限（账号 / 令牌走 `os.environ`，禁止硬编码，受 gitleaks 拦截）。
- 网络下载通道建议配置合理限流与重试；增量模式注意服务商时间窗限制。

### 3.3 纸面交易对接妙算后台

妙算内的**纸面交易**同样**对接妙算后台**：以妙算后台提供的实时 / 仿真行情与执行回执为输入，在妙算侧做信号复现与绩效仿真，**不**独立搭建行情源。

- 纸面交易的行情源、成交回执、账户状态均经妙算后台只读接口回流，与正式上线共享同一数据通路。
- 这保证了「纸面」与「实盘」在数据口径、成本模型、时间戳语义上完全一致，纸面验证通过的策略可直接移交妙算上线，避免环境偏差。
...
*当前进度：T01–T07 完成（工程地基 + vocab 移植 + ops/features numpy 化 + 端到端对拍 + 妙算适配器 + 回测 + Web UI）。*
