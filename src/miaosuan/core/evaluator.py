"""因子有效性评估器（numpy 化移植，M5）—— 打分 / 剪枝 / 消融 / 报告 / Active_Subset。

移植自冻结 AlphaMaster 的 ``model_core/evaluator.py``（原 torch 实现），逐行对齐语义
（R4–R7）：

* :func:`_align_causal` —— 防 look-ahead 的时间对齐（**原样保留**，末端裁掉 ``horizon`` 步）；
* :func:`_compute_ic_rankic` / :func:`_compute_mi` / :func:`_is_degenerate` / :func:`_pearson_corr`；
* :func:`score` / :func:`score_all` / :func:`prune` / :func:`ablate` / :func:`build_report` /
  :func:`select_active_subset` 与 :class:`EffectivenessEvaluator`。

关键等价与边界拆分：

* ``torch`` 张量 → numpy 数组（``cand`` / ``tgt`` 直接为 ``np.ndarray``）；IC/RankIC 仍用
  ``scipy.stats.pearsonr`` / ``spearmanr``（与 AM 同一实现，数值逐点一致）；MI 用等频分箱。
* **可复现**：本模块无全局可变状态；``build_report`` 的 ``generated_at`` 改为**可注入**
  （默认仍取 UTC now，便于对拍时冻结时间戳）。
* **无 IO**：``save_report`` / ``load_report``（AM 中做文件读写）已拆到
  :mod:`miaosuan.report.persist`，``core/`` 保持纯 numpy、无文件 IO。
* ``ConfigError`` 统一取自 :mod:`miaosuan.errors`（错误码 ``E-CONFIG``）。
"""

from __future__ import annotations

import math
import warnings
from dataclasses import dataclass
from datetime import UTC, datetime
from typing import Any

import numpy as np
from scipy.stats import pearsonr, spearmanr

from ..errors import ConfigError

__all__ = [
    "AblationResult",
    "ConfigError",
    "EffectivenessEvaluator",
    "Report",
    "ReportRow",
    "ScoreResult",
    "ablate",
    "build_report",
    "prune",
    "score",
    "score_all",
    "select_active_subset",
]


# ── 数据模型 ──────────────────────────────────────────────────────────────


@dataclass
class ScoreResult:
    """单个候选的打分结果（R4）。

    ``importance_score`` 为聚合分数；``unscorable=True`` 表示序列退化/样本不足，
    此时 ``importance_score`` 为 ``-inf``（哨兵，排序用，绝不产 NaN）。
    """

    candidate: str
    category: str
    ic: float
    rank_ic: float
    ir: float
    mi: float
    importance_score: float
    unscorable: bool


@dataclass
class ReportRow(ScoreResult):
    """打分结果扩展：含剪枝状态与消融结果（R7.2）。"""

    retention_status: str
    pruned_in_favor_of: str | None
    marginal_contribution: float | None
    drop_recommendation: bool


@dataclass
class AblationResult:
    """单元素消融结果（R6）。"""

    name: str
    marginal_contribution: float | None
    drop_recommendation: bool
    error: str | None


@dataclass
class Report:
    """排序报告（R7.5）。"""

    vocab_version: str
    generated_at: str
    config: dict[str, Any]
    rows: list[ReportRow]
    active_subset: list[str]


# ── 内部工具函数 ────────────────────────────────────────────────────────────


def _as_array(x: np.ndarray) -> np.ndarray:
    """保证为 numpy 数组（AM ``_to_numpy`` 的去 torch 版）。"""
    return np.asarray(x)


def _align_causal(
    candidate: np.ndarray,
    target: np.ndarray,
    horizon: int,
) -> tuple[np.ndarray, np.ndarray]:
    """防 look-ahead 的时间对齐（R4.7，**原样保留**）。

    candidate: ``[N, T]`` 或 ``[1, T]``；target: ``[N, T]``（``target[t]`` 代表 t→t+h）。

    配对规则：把 ``candidate[t]`` 与 ``target[t]`` 配对，末端裁掉最后 ``h`` 步以避免用到
    尚未实现的收益，返回 ``[N, T-h]``。严格防 look-ahead：只用 ≤t 的 candidate 配对
    ``target[t]``，而非 ``target[t+h]``——因为 ``target[t]`` 本身就代表 t→t+h 的收益。
    """
    horizon = max(1, int(horizon))
    t = target.shape[-1]
    if t <= horizon:
        empty_c = candidate[..., :0]
        empty_t = target[..., :0]
        return empty_c, empty_t
    cand_aligned = candidate[..., : t - horizon]
    tgt_aligned = target[..., : t - horizon]
    return cand_aligned, tgt_aligned


def _compute_ic_rankic(cand: np.ndarray, tgt: np.ndarray) -> tuple[float, float, float]:
    """计算 IC（Pearson）与 RankIC（Spearman），返回 ``(ic_mean, rank_ic_mean, ir)``。

    对每个品种计算时间维相关系数，再跨品种取均值；``IR = mean(IC)/std(IC)``。
    """
    cand_np = _as_array(cand)
    tgt_np = _as_array(tgt)
    n = cand_np.shape[0]

    ic_list: list[float] = []
    rank_ic_list: list[float] = []

    for i in range(n):
        c = cand_np[i]
        t_ = tgt_np[i]
        mask = np.isfinite(c) & np.isfinite(t_)
        if mask.sum() < 2:
            continue
        cv, tv = c[mask], t_[mask]
        if cv.std() < 1e-8 or tv.std() < 1e-8:
            continue
        try:
            ic_val, _ = pearsonr(cv, tv)
            ric_val, _ = spearmanr(cv, tv)
        except Exception:  # noqa: BLE001 - 与 AM 一致：单品种失败跳过
            continue
        if math.isfinite(ic_val):
            ic_list.append(float(ic_val))
        if math.isfinite(ric_val):
            rank_ic_list.append(float(ric_val))

    if len(ic_list) == 0:
        return 0.0, 0.0, 0.0

    ic_mean = float(np.mean(ic_list))
    ric_mean = float(np.mean(rank_ic_list)) if rank_ic_list else 0.0
    ic_std = float(np.std(ic_list))
    ir = ic_mean / (ic_std + 1e-10) if ic_std > 1e-10 else 0.0
    return ic_mean, ric_mean, ir


def _compute_mi(cand: np.ndarray, tgt: np.ndarray) -> float:
    """计算离散互信息（等频分箱，自适应箱数）。

    箱数 = ``max(5, min(20, int(sqrt(T_eff))))``；返回 MI（≥0），退化/样本不足返回 0.0。
    """
    cand_np = _as_array(cand)
    tgt_np = _as_array(tgt)

    c_flat = cand_np.flatten()
    t_flat = tgt_np.flatten()

    mask = np.isfinite(c_flat) & np.isfinite(t_flat)
    n_valid = int(mask.sum())
    if n_valid < 2:
        return 0.0

    cv, tv = c_flat[mask], t_flat[mask]

    if cv.std() < 1e-8:
        return 0.0

    n_bins = max(5, min(20, int(math.sqrt(n_valid))))

    try:
        c_q = np.percentile(cv, np.linspace(0, 100, n_bins + 1))
        t_q = np.percentile(tv, np.linspace(0, 100, n_bins + 1))
        c_q = np.unique(c_q)
        t_q = np.unique(t_q)
        c_bins = np.searchsorted(c_q[1:-1], cv, side="right")
        t_bins = np.searchsorted(t_q[1:-1], tv, side="right")
    except Exception:  # noqa: BLE001 - 与 AM 一致：分箱失败返回 0
        return 0.0

    n_c = len(c_q)
    n_t = len(t_q)
    joint = np.zeros((n_c, n_t), dtype=np.float64)
    for ci, ti in zip(c_bins, t_bins, strict=False):
        joint[ci, ti] += 1.0
    joint /= joint.sum() + 1e-15

    pc = joint.sum(axis=1)
    pt = joint.sum(axis=0)

    mi = 0.0
    for i in range(n_c):
        for j in range(n_t):
            pij = joint[i, j]
            if pij > 0 and pc[i] > 0 and pt[j] > 0:
                mi += pij * math.log(pij / (pc[i] * pt[j]))

    return max(0.0, float(mi))


def _is_degenerate(cand: np.ndarray) -> bool:
    """判断候选序列是否退化（``std < 1e-8`` 或有效样本 < 2）。"""
    c = _as_array(cand).flatten()
    mask = np.isfinite(c)
    if mask.sum() < 2:
        return True
    valid = c[mask]
    return float(valid.std()) < 1e-8


def _pearson_corr(a: np.ndarray, b: np.ndarray) -> float:
    """两个序列的 Pearson 相关绝对值；重叠 < 2 或零方差 → 0.0（保守不剪）。"""
    a_np = _as_array(a).flatten()
    b_np = _as_array(b).flatten()

    n = min(len(a_np), len(b_np))
    a_np, b_np = a_np[:n], b_np[:n]

    mask = np.isfinite(a_np) & np.isfinite(b_np)
    if mask.sum() < 2:
        return 0.0
    av, bv = a_np[mask], b_np[mask]
    if av.std() < 1e-8 or bv.std() < 1e-8:
        return 0.0
    try:
        r, _ = pearsonr(av, bv)
        return abs(float(r)) if math.isfinite(r) else 0.0
    except Exception:  # noqa: BLE001 - 与 AM 一致
        return 0.0


def _rank_normalize(values: list[float]) -> list[float]:
    """将值列表秩归一化到 ``[0, 1]``（-inf / nan 哨兵值得到 0.0）。"""
    n = len(values)
    if n == 0:
        return []
    arr = np.array(values, dtype=float)
    valid_mask = np.isfinite(arr) & (arr > -1e30)
    result = np.zeros(n, dtype=float)
    valid_indices = np.where(valid_mask)[0]
    if len(valid_indices) == 0:
        return result.tolist()
    valid_vals = arr[valid_indices]
    order = np.argsort(np.argsort(valid_vals))
    n_valid = len(valid_vals)
    if n_valid == 1:
        result[valid_indices[0]] = 1.0
    else:
        result[valid_indices] = order / (n_valid - 1)
    return result.tolist()


# ── 模块级独立函数（与 EffectivenessEvaluator 解耦，便于测试）───────────────


def score(
    candidate: np.ndarray,
    target: np.ndarray,
    name: str,
    category: str = "",
    w_rankic: float = 0.6,
    w_mi: float = 0.4,
    eval_window: int = 250,
    horizon: int = 1,
) -> ScoreResult:
    """对单个候选打分（R4）。单次调用无跨候选归一，``importance_score`` 用 ``|rank_ic|`` 代理。"""
    cand_a, tgt_a = _align_causal(candidate, target, horizon)

    if _is_degenerate(cand_a):
        return ScoreResult(
            candidate=name,
            category=category,
            ic=0.0,
            rank_ic=0.0,
            ir=0.0,
            mi=0.0,
            importance_score=float("-inf"),
            unscorable=True,
        )

    ic_mean, ric_mean, ir = _compute_ic_rankic(cand_a, tgt_a)
    mi_val = _compute_mi(cand_a, tgt_a)
    importance_score = w_rankic * abs(ric_mean) + w_mi * mi_val

    return ScoreResult(
        candidate=name,
        category=category,
        ic=ic_mean,
        rank_ic=ric_mean,
        ir=ir,
        mi=mi_val,
        importance_score=importance_score,
        unscorable=False,
    )


def score_all(
    candidates_dict: dict[str, np.ndarray],
    target: np.ndarray,
    categories: dict[str, str] | None = None,
    w_rankic: float = 0.6,
    w_mi: float = 0.4,
    eval_window: int = 250,
    horizon: int = 1,
) -> list[ScoreResult]:
    """对所有候选打分并做跨候选秩归一（R4.4）。退化序列保留且排最后（``-inf``）。"""
    categories = categories or {}
    raw: list[ScoreResult] = []

    for name, cand in candidates_dict.items():
        cat = categories.get(name, "")
        sr = score(
            candidate=cand,
            target=target,
            name=name,
            category=cat,
            w_rankic=w_rankic,
            w_mi=w_mi,
            eval_window=eval_window,
            horizon=horizon,
        )
        raw.append(sr)

    abs_rankic = [abs(r.rank_ic) if not r.unscorable else float("-inf") for r in raw]
    mi_vals = [r.mi if not r.unscorable else float("-inf") for r in raw]

    norm_rankic = _rank_normalize(abs_rankic)
    norm_mi = _rank_normalize(mi_vals)

    results: list[ScoreResult] = []
    for i, sr in enumerate(raw):
        agg = (
            float("-inf")
            if sr.unscorable
            else w_rankic * norm_rankic[i] + w_mi * norm_mi[i]
        )
        results.append(
            ScoreResult(
                candidate=sr.candidate,
                category=sr.category,
                ic=sr.ic,
                rank_ic=sr.rank_ic,
                ir=sr.ir,
                mi=sr.mi,
                importance_score=agg,
                unscorable=sr.unscorable,
            )
        )

    return results


def prune(
    scores: list[ScoreResult],
    series_dict: dict[str, np.ndarray],
    corr_threshold: float = 0.9,
    conservative: bool = False,
    margin: float = 0.01,
) -> list[ReportRow]:
    """保守双条件相关性剪枝（R5）。

    ``conservative=True`` 时阈值上调到 0.95；``margin`` 为分差阈值（势均力敌时保留）。
    """
    if not (0.0 <= corr_threshold <= 1.0):
        raise ConfigError(
            f"corr_threshold={corr_threshold} 越界，必须在 [0.0, 1.0] 内",
            context={"corr_threshold": corr_threshold},
        )

    threshold = 0.95 if conservative else corr_threshold

    def sort_key(sr: ScoreResult) -> tuple[float, str]:
        s = sr.importance_score
        if not math.isfinite(s):
            s = float("-inf")
        return (-s, sr.candidate)

    ordered = sorted(scores, key=sort_key)

    retained: list[ScoreResult] = []
    rows: list[ReportRow] = []

    name_to_series = series_dict

    for sr in ordered:
        pruned_by: str | None = None
        for ret in retained:
            a_series = name_to_series.get(sr.candidate)
            b_series = name_to_series.get(ret.candidate)
            if a_series is None or b_series is None:
                continue
            corr = _pearson_corr(a_series, b_series)
            if corr < threshold:
                continue
            s_retained = ret.importance_score
            s_current = sr.importance_score
            if not math.isfinite(s_retained):
                s_retained = float("-inf")
            if not math.isfinite(s_current):
                s_current = float("-inf")
            if s_retained - s_current > margin:
                pruned_by = ret.candidate
                break
        if pruned_by is not None:
            rows.append(
                ReportRow(
                    candidate=sr.candidate,
                    category=sr.category,
                    ic=sr.ic,
                    rank_ic=sr.rank_ic,
                    ir=sr.ir,
                    mi=sr.mi,
                    importance_score=sr.importance_score,
                    unscorable=sr.unscorable,
                    retention_status="pruned",
                    pruned_in_favor_of=pruned_by,
                    marginal_contribution=None,
                    drop_recommendation=False,
                )
            )
        else:
            retained.append(sr)
            rows.append(
                ReportRow(
                    candidate=sr.candidate,
                    category=sr.category,
                    ic=sr.ic,
                    rank_ic=sr.rank_ic,
                    ir=sr.ir,
                    mi=sr.mi,
                    importance_score=sr.importance_score,
                    unscorable=sr.unscorable,
                    retention_status="retained",
                    pruned_in_favor_of=None,
                    marginal_contribution=None,
                    drop_recommendation=False,
                )
            )

    return rows


def _compute_metric(
    candidates_dict: dict[str, np.ndarray],
    target: np.ndarray,
    horizon: int,
    w_rankic: float,
    w_mi: float,
) -> float:
    """候选集合的整体 metric（``|IC|`` 绝对值均值），供 ablate 内部使用。"""
    if not candidates_dict:
        return 0.0
    ic_vals: list[float] = []
    for _name, cand in candidates_dict.items():
        cand_a, tgt_a = _align_causal(cand, target, horizon)
        if _is_degenerate(cand_a):
            continue
        ic_mean, _ric_mean, _ir = _compute_ic_rankic(cand_a, tgt_a)
        ic_vals.append(abs(ic_mean))
    if not ic_vals:
        return 0.0
    return float(sum(ic_vals) / len(ic_vals))


def ablate(
    name: str,
    base_set: dict[str, np.ndarray],
    target: np.ndarray,
    drop_threshold: float = -0.01,
    n_windows: int = 5,
    horizon: int = 1,
    w_rankic: float = 0.6,
    w_mi: float = 0.4,
) -> AblationResult:
    """单元素消融（R6）。``base_set`` 必须包含被消融项 ``name``。"""
    if name not in base_set:
        return AblationResult(
            name=name,
            marginal_contribution=None,
            drop_recommendation=False,
            error=f"候选 '{name}' 不在 base_set 中",
        )

    without_set = {k: v for k, v in base_set.items() if k != name}

    t_full = target.shape[-1]
    if t_full < 2:
        return AblationResult(
            name=name,
            marginal_contribution=None,
            drop_recommendation=False,
            error="target 序列过短，无法计算 metric",
        )

    window_size = max(2, t_full // n_windows)
    marginals: list[float] = []

    for w_idx in range(n_windows):
        start = w_idx * (t_full // n_windows)
        end = start + window_size
        if end > t_full:
            end = t_full
        if end - start < 2:
            continue

        tgt_w = target[..., start:end]
        with_dict = {k: v[..., start:end] for k, v in base_set.items()}
        without_dict = {k: v[..., start:end] for k, v in without_set.items()}

        try:
            m_with = _compute_metric(with_dict, tgt_w, horizon, w_rankic, w_mi)
        except Exception as e:  # noqa: BLE001 - 与 AM 一致：失败即返回 error
            return AblationResult(
                name=name,
                marginal_contribution=None,
                drop_recommendation=False,
                error=f"含候选配置计算失败（窗口{w_idx}）: {e}",
            )

        try:
            m_without = _compute_metric(without_dict, tgt_w, horizon, w_rankic, w_mi)
        except Exception as e:  # noqa: BLE001
            return AblationResult(
                name=name,
                marginal_contribution=None,
                drop_recommendation=False,
                error=f"去候选配置计算失败（窗口{w_idx}）: {e}",
            )

        marginals.append(m_with - m_without)

    if not marginals:
        return AblationResult(
            name=name,
            marginal_contribution=None,
            drop_recommendation=False,
            error="所有窗口均无法计算 marginal",
        )

    marginal = float(sum(marginals) / len(marginals))
    drop_rec = marginal <= drop_threshold

    return AblationResult(
        name=name,
        marginal_contribution=marginal,
        drop_recommendation=drop_rec,
        error=None,
    )


def build_report(
    rows: list[ReportRow],
    active_subset: list[str],
    vocab_version: str,
    config: dict[str, Any],
    generated_at: str | None = None,
) -> Report:
    """构建排序报告（R7.1）。按 ``importance_score`` 降序 + 确定性 tie-break（名称字母序）。

    ``generated_at`` 可注入（默认 UTC now），便于对拍时冻结时间戳。
    """

    def sort_key(row: ReportRow) -> tuple[float, str]:
        s = row.importance_score
        if not math.isfinite(s):
            s = float("-inf")
        return (-s, row.candidate)

    sorted_rows = sorted(rows, key=sort_key)
    if generated_at is None:
        generated_at = datetime.now(UTC).isoformat()

    return Report(
        vocab_version=vocab_version,
        generated_at=generated_at,
        config=config,
        rows=sorted_rows,
        active_subset=list(active_subset),
    )


def row_to_dict(row: ReportRow) -> dict[str, Any]:
    """将 ReportRow 序列化为 JSON-ready dict（``marginal_contribution=None`` → ``"not_computed"``）。"""
    return {
        "candidate": row.candidate,
        "category": row.category,
        "ic": row.ic,
        "rank_ic": row.rank_ic,
        "ir": row.ir,
        "mi": row.mi,
        "importance_score": (
            row.importance_score if math.isfinite(row.importance_score) else None
        ),
        "unscorable": row.unscorable,
        "retention_status": row.retention_status,
        "pruned_in_favor_of": row.pruned_in_favor_of,
        "marginal_contribution": (
            "not_computed" if row.marginal_contribution is None else row.marginal_contribution
        ),
        "drop_recommendation": row.drop_recommendation,
    }


def dict_to_row(d: dict[str, Any]) -> ReportRow:
    """从 JSON dict 反序列化 ReportRow（``"not_computed"`` → ``marginal_contribution=None``）。"""
    mc_raw = d.get("marginal_contribution", "not_computed")
    mc: float | None = None if mc_raw == "not_computed" else float(mc_raw)

    imp = d.get("importance_score")
    importance_score = float("-inf") if imp is None else float(imp)

    pruned = d.get("pruned_in_favor_of")
    return ReportRow(
        candidate=d["candidate"],
        category=d.get("category", ""),
        ic=float(d.get("ic", 0.0)),
        rank_ic=float(d.get("rank_ic", 0.0)),
        ir=float(d.get("ir", 0.0)),
        mi=float(d.get("mi", 0.0)),
        importance_score=importance_score,
        unscorable=bool(d.get("unscorable", False)),
        retention_status=d.get("retention_status", "retained"),
        pruned_in_favor_of=None if pruned is None else str(pruned),
        marginal_contribution=mc,
        drop_recommendation=bool(d.get("drop_recommendation", False)),
    )


def select_active_subset(
    report: Report,
    retention_threshold: float = 0.0,
    max_retained: int | None = None,
) -> list[str]:
    """选择 Active_Subset（R7.3/7.4/7.7）。默认语义：全保留（``threshold=0.0``, 无上限）。

    空选择 → 返回空列表并发出 warning（不抛错，R7.7）。
    """

    def sort_key(row: ReportRow) -> tuple[float, str]:
        s = row.importance_score
        if not math.isfinite(s):
            s = float("-inf")
        return (-s, row.candidate)

    retained_rows = [r for r in report.rows if r.retention_status == "retained"]
    retained_sorted = sorted(retained_rows, key=sort_key)

    def _passes_threshold(r: ReportRow) -> bool:
        if r.unscorable:
            # 保守：默认阈值 0.0 下 unscorable 亦保留。
            return retention_threshold <= 0.0
        return math.isfinite(r.importance_score) and r.importance_score >= retention_threshold

    filtered = [r for r in retained_sorted if _passes_threshold(r)]

    if max_retained is not None:
        filtered = filtered[:max_retained]

    if not filtered:
        warnings.warn(
            "select_active_subset: Active_Subset 为空，所有候选均未通过阈值筛选。",
            UserWarning,
            stacklevel=2,
        )

    return [r.candidate for r in filtered]


# ── 主类 EffectivenessEvaluator ─────────────────────────────────────────


class EffectivenessEvaluator:
    """Effectiveness_Evaluator 主类（R4–R7）。

    封装打分、剪枝、消融、报告构建与 Active_Subset 选择。所有默认参数遵循
    「宁可放过，不要错杀」的保守原则。

    :param corr_threshold: 相关性剪枝阈值，默认 0.9；``conservative=True`` 时 0.95。
    :param conservative: 保守模式，上调剪枝阈值。
    :param retention_threshold: Active_Subset 分数下界，默认 0.0（全保留）。
    :param max_retained: Active_Subset 最大候选数，默认 None（不设上限）。
    :param drop_threshold: 消融 drop 建议阈值（带负值保护带），默认 -0.01。
    :param w_rankic: RankIC 权重，默认 0.6。
    :param w_mi: MI 权重，默认 0.4。
    :param eval_window: 评估窗口大小，默认 250（保留参数）。
    :param target_horizon: 前视收益 horizon（``_align_causal`` 用），默认 2。
    """

    def __init__(
        self,
        corr_threshold: float = 0.9,
        conservative: bool = False,
        retention_threshold: float = 0.0,
        max_retained: int | None = None,
        drop_threshold: float = -0.01,
        w_rankic: float = 0.6,
        w_mi: float = 0.4,
        eval_window: int = 250,
        target_horizon: int = 2,
    ) -> None:
        if not (0.0 <= corr_threshold <= 1.0):
            raise ConfigError(
                f"corr_threshold={corr_threshold} 越界，必须在 [0.0, 1.0] 内",
                context={"corr_threshold": corr_threshold},
            )
        self.corr_threshold = corr_threshold
        self.conservative = conservative
        self.retention_threshold = retention_threshold
        self.max_retained = max_retained
        self.drop_threshold = drop_threshold
        self.w_rankic = w_rankic
        self.w_mi = w_mi
        self.eval_window = eval_window
        self.target_horizon = target_horizon

    # ── 打分 ────────────────────────────────────────────────────────────

    def score(
        self,
        candidate: np.ndarray,
        target: np.ndarray,
        name: str,
        category: str = "",
    ) -> ScoreResult:
        """对单个候选打分（R4）。"""
        return score(
            candidate=candidate,
            target=target,
            name=name,
            category=category,
            w_rankic=self.w_rankic,
            w_mi=self.w_mi,
            eval_window=self.eval_window,
            horizon=self.target_horizon,
        )

    def score_all(
        self,
        candidates: dict[str, np.ndarray],
        target: np.ndarray,
        categories: dict[str, str] | None = None,
    ) -> list[ScoreResult]:
        """对所有候选打分并做跨候选秩归一（R4）。"""
        return score_all(
            candidates_dict=candidates,
            target=target,
            categories=categories,
            w_rankic=self.w_rankic,
            w_mi=self.w_mi,
            eval_window=self.eval_window,
            horizon=self.target_horizon,
        )

    # ── 剪枝 ────────────────────────────────────────────────────────────

    def prune(
        self,
        scores: list[ScoreResult],
        series: dict[str, np.ndarray],
    ) -> list[ReportRow]:
        """保守双条件相关性剪枝（R5）。"""
        return prune(
            scores=scores,
            series_dict=series,
            corr_threshold=self.corr_threshold,
            conservative=self.conservative,
        )

    # ── 消融 ────────────────────────────────────────────────────────────

    def ablate(
        self,
        name: str,
        base_set: dict[str, np.ndarray],
        target: np.ndarray,
    ) -> AblationResult:
        """单元素消融（R6）。"""
        return ablate(
            name=name,
            base_set=base_set,
            target=target,
            drop_threshold=self.drop_threshold,
            horizon=self.target_horizon,
            w_rankic=self.w_rankic,
            w_mi=self.w_mi,
        )

    # ── 报告 ────────────────────────────────────────────────────────────

    def build_report(
        self,
        rows: list[ReportRow],
        active_subset: list[str],
        generated_at: str | None = None,
    ) -> Report:
        """构建排序报告（R7.1）。"""
        from .vocab import VOCAB_VERSION

        config_snapshot = {
            "corr_threshold": self.corr_threshold,
            "conservative": self.conservative,
            "retention_threshold": self.retention_threshold,
            "max_retained": self.max_retained,
            "drop_threshold": self.drop_threshold,
            "w_rankic": self.w_rankic,
            "w_mi": self.w_mi,
            "eval_window": self.eval_window,
            "target_horizon": self.target_horizon,
        }
        return build_report(
            rows=rows,
            active_subset=active_subset,
            vocab_version=VOCAB_VERSION,
            config=config_snapshot,
            generated_at=generated_at,
        )

    # ── Active_Subset ───────────────────────────────────────────────────

    def select_active_subset(self, report: Report) -> list[str]:
        """选择 Active_Subset（R7.3/7.4/7.7）。默认全保留。"""
        return select_active_subset(
            report=report,
            retention_threshold=self.retention_threshold,
            max_retained=self.max_retained,
        )

    # ── 静态工具 ────────────────────────────────────────────────────────

    @staticmethod
    def _align_causal(
        candidate: np.ndarray,
        target: np.ndarray,
        horizon: int,
    ) -> tuple[np.ndarray, np.ndarray]:
        """防 look-ahead 的时间对齐（R4.7）。"""
        return _align_causal(candidate, target, horizon)
