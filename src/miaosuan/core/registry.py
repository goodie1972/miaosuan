...
校验与异常映射（对应 AM requirements R10.5–R10.8）：
  ...
  情形                               异常
  ================================  =========================
  重复名                             :class:`DuplicateNameError`   (R10.5)
  声明 arity 与 transform 实际操作数不符  :class:`ArityMismatchError`   (R10.6)
  缺必填字段                          :class:`MissingFieldError`    (R10.7)
  arity 非 0..10 整数                 :class:`InvalidArityError`    (R10.8)
  ================================  =========================
...
class FeatureSpec:
    ...
    compute:  计算函数，签名 ``(raw_dict: dict) -> ndarray[N, T]``。
    """
...
class OperatorSpec:
    ...
    transform: 变换函数，签名 ``(*operands: ndarray[N, T]) -> ndarray[N, T]``。
    """
...
def _observed_arity(transform: Callable[..., Any]) -> int | None:
...
class Registry:
    ...
    @staticmethod
    def _require_fields(spec: object, fields: dict[str, str]) -> None:
    ...
    @staticmethod
    def _validate_arity(arity: object) -> None:
