...
由注册层（:mod:`miaosuan.core.registry.Registry`）驱动、版本确定性派生的实现：
  ...
  - ``feature_names`` 来自 :data:`miaosuan.core.features.FEATURE_REGISTRY`（有序）。
  - ``operator_names`` 来自 :data:`miaosuan.core.ops.OPERATOR_REGISTRY`（有序）。
  ...
  - ``FORMULA_VOCAB.verify(artifact_version)``：版本不匹配抛
    :class:`VocabVersionMismatchError`，拒绝且不消费任何 token（R3.7）。
...
from ..errors import VocabVersionMismatchError
...
class FormulaVocab:
    ...
    def verify(self, artifact_version: str) -> None:
        ...
        不匹配抛 :class:`VocabVersionMismatchError`，调用方据此拒绝加载、不消费
        任何 token。匹配则静默返回。
        """
